...
PATCHES_PATH=${PROJECT_ROOT}/device/board/xiaomi/sagit/patches
HDF_PATCH_FILE=${PATCHES_PATH}/hdf.patch
KERNEL_PATCH=${PATCHES_PATH}/sagit.patch

echo "start check_patch.sh"
...
if [ ! -d "${KERNEL_SRC_TMP_PATH}" ];then
    ...
    #kernel patch
    [ -e "${KERNEL_PATCH}" ] && patch -p1 < ${KERNEL_PATCH}
fi
